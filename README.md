# Better Plain Text

This Backdrop CMS module came into existence as a result of my own frustrations in figuring out how to set up a multi-line
Plain text field which respects line breaks upon display. The prototypical use case is a street address, where no
"rich text" formatting is needed, no wysiwyg editor is needed, but line breaks are an integral part of the format
to be displayed.

Also see this discussion: https://github.com/backdrop/backdrop-issues/issues/5515 for further insight on the issue.

## Usage

### After enabling this module, you will see four formatting options when "Plain text" processing is selected:

<img width="817" height="251" alt="image" src="https://github.com/user-attachments/assets/ff5d5844-a5bc-4fcc-9bbd-005fc77c9396" />


### You will also find these options for overriding the display for any given display:

<img width="907" height="381" alt="image" src="https://github.com/user-attachments/assets/4fc9d18f-2a1d-415c-b45d-0e264f42700f" />


### ...as well as for Views:

<img width="705" height="424" alt="image" src="https://github.com/user-attachments/assets/62b1fa1d-1004-41e0-b489-d8f054d30e79" />

### Note that no html is introduced into the stored plaintext string in order to get these results.
It works by embedding the string in a `<span style="white-space: pre-wrap;>field_text_here</span>`
(or "white-space: pre-line") prior to output. The browser then honors the linebreaks ("\n" characters) as linefeeds.

## Enjoy.

—Eric
